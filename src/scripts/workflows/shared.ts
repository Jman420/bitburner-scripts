...
async function infiniteLoop(loopFunction: LoopableFunction, netscript: NS, logWriter: Logger) {
  /* eslint-disable-next-line no-constant-condition */
  while (true) {
    await loopFunction(netscript, logWriter);

    const sleepTime = randomIntWithinRange(MIN_LOOP_SLEEP_MILLISEC, MAX_LOOP_SLEEP_MILLISEC);
    await netscript.sleep(sleepTime);
  }
}

export {CMD_ARG_PREFIX, HOME_SERVER_NAME, MIN_LOOP_SLEEP_MILLISEC, MAX_LOOP_SLEEP_MILLISEC, getCmdArgFlag, randomIntWithinRange, infiniteLoop};
