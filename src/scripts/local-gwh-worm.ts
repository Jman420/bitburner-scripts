...
async function attackNetwork(netscript: NS, logWriter: Logger) {
  ...
  for (const hostname of hosts) {
    ...
    if (rootAccess) {
      if (!netscript.isRunning(WORM_SCRIPT, hostname) && !netscript.isRunning(ATTACK_SCRIPT, hostname)) {
...
export async function main(netscript: NS) {
  ...
  const runnerDelay = randomIntWithinRange(RUNNER_SCRIPT_MIN_DELAY, RUNNER_SCRIPT_MAX_DELAY);
  const attackDelay = randomIntWithinRange(RUNNER_SCRIPT_MIN_DELAY, RUNNER_SCRIPT_MAX_DELAY);
  const threadCount = maxScriptThreads(netscript, hostname, ATTACK_SCRIPT, true, RUNNER_SCRIPT);
  logWriter.writeLine(`Spawning Runner Script...`);
  if (!spawnScript(netscript, RUNNER_SCRIPT, runnerDelay, false, getCmdArgFlag(CMD_ARG_SCRIPT_PATH), ATTACK_SCRIPT, getCmdArgFlag(CMD_ARG_DELAY), attackDelay, getCmdArgFlag(CMD_ARG_THREAD_COUNT), threadCount)) {
